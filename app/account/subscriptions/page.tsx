      ...
      <main className="flex-1 bg-gray-50 py-12">
        <div className="container px-4 md:px-6">
          ...
          {subscriptions.length > 0 ? (
                    ...
                    <Card key={subscription.id}>
                      <CardHeader className="pb-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <CardTitle>{products[subscription.productId].name}</CardTitle>
                      ...
                      <CardContent>
                        <div className="flex items-center gap-4 mb-4">
                          <div className="w-16 h-16 relative shrink-0 rounded overflow-hidden">
                            <Image
                              src={products[subscription.productId].image || "/placeholder.svg"}
                      ...
                      {subscriptions.map((subscription) => (
                        <tr key={`history-${subscription.id}`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <div className="text-sm text-gray-500">${subscription.price.toFixed(2)}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="bg-white rounded-lg shadow-sm p-8 text-center">
              ...
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                You don't have any active subscriptions yet. Subscribe to Vitalis products for regular delivery and
                save.
              </p>
              <Button
                className="bg-emerald-700 hover:bg-emerald-800"
          ...
          <div className="mt-16">
            ...
            <Tabs value={selectedFrequency} onValueChange={(v) => setSelectedFrequency(v as "monthly" | "annual")}>
              ...
              <TabsContent value="monthly" className="space-y-6">
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                  {Object.values(products).map((product) => (
                    <Card key={`monthly-${product.id}`} className="overflow-hidden">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-emerald-700">${product.price.toFixed(2)}/month</div>
                        <p className="text-sm text-gray-500 mt-1">Billed monthly</p>
                      </CardContent>
                      <CardFooter>
                        <Button className="w-full bg-emerald-700 hover:bg-emerald-800">Subscribe</Button>
                      </CardFooter>
                    </Card>
                  ))}
                </div>
              ...
              <TabsContent value="annual" className="space-y-6">
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                  {Object.values(products).map((product) => {
                    const annualPrice = getSubscriptionPrice(product.id as ProductId, "annual")
                    const monthlyEquivalent = (annualPrice / 12).toFixed(2)

                    return (
                      <Card key={`annual-${product.id}`} className="overflow-hidden">
                        <div className="aspect-square relative">
                          <Image
                            src={product.image || "/placeholder.svg"}
                            alt={product.name}
                        ...
                        <CardContent>
                          <div className="text-2xl font-bold text-emerald-700">${annualPrice.toFixed(2)}/year</div>
                          <p className="text-sm text-gray-500 mt-1">${monthlyEquivalent}/month billed annually</p>
